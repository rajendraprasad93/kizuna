import type { AiAnalysis, PossibleCause, RecommendedAction, ProblemCategory } from '@/types';

const PROBLEM_KEYWORDS: Record<string, string[]> = {
  flooding: ['water', 'flood', 'waterlog', 'rain', 'drainage', 'drain', 'standing water', 'overflow', 'puddle', 'submerge', 'inundat'],
  pothole: ['pothole', 'crack', 'road', 'surface', 'damage', 'broken', 'rough', 'dip', 'rut', 'asphalt', 'pavement'],
  garbage: ['garbage', 'trash', 'waste', 'rubbish', 'dump', 'litter', 'pile', 'uncollected', 'rubbish', 'debris', 'dumping'],
};

const CONDITIONS_BY_PROBLEM: Record<string, string[]> = {
  flooding: ['standing_water', 'blocked_drain', 'road_submersion', 'poor_drainage', 'heavy_sediment'],
  pothole: ['surface_cracking', 'road_degradation', 'void_in_surface', 'edge_cracking', 'alligator_cracking'],
  garbage: ['waste_accumulation', 'overflowing_bin', 'scattered_debris', 'organic_waste', 'improper_dumping'],
};
  ...
  flooding: [
    {
      ...
      confidence: 0.82,
      evidence: ['7 previous flooding reports within 500m', '3 drainage complaints nearby', 'No rainfall in last 72 hours'],
      severity: 'high',
      urgency: 'immediate',
    },
    {
      ...
      confidence: 0.65,
      evidence: ['Historical flooding pattern at this location', 'Older drainage infrastructure (15+ years)'],
      severity: 'medium',
      urgency: 'short-term',
    },
    {
      ...
      confidence: 0.48,
      evidence: ['Natural water accumulation zone', 'Surrounding gradient channels water here'],
      severity: 'low',
      urgency: 'long-term',
    },
  ],
  pothole: [
    {
      ...
      confidence: 0.78,
      evidence: ['Recent rainfall events', 'Visible surface cracking allowing water ingress', 'Temperature fluctuations'],
      severity: 'high',
      urgency: 'short-term',
    },
    {
    ...
    },
    {
      ...
      confidence: 0.55,
      evidence: ['Road resurfacing not done in 8+ years', 'Widespread surface degradation'],
      severity: 'medium',
      urgency: 'long-term',
    },
  ],
  garbage: [
    {
      ...
      confidence: 0.79,
      evidence: ['3 previous garbage reports on this street', 'Collection delay reported by neighbors'],
      severity: 'high',
      urgency: 'immediate',
    },
    {
      ...
      confidence: 0.58,
      evidence: ['High-density residential area', 'Overflowing bins visible in nearby reports'],
      severity: 'medium',
      urgency: 'short-term',
    },
    {
      ...
      confidence: 0.45,
      evidence: ['Construction debris type waste', 'Reports cluster during nighttime hours'],
      severity: 'medium',
      urgency: 'short-term',
    },
  ],
  ...
  flooding: [
    {
      ...
      steps: ['Dispatch inspection team', 'Identify blockage points', 'Clear debris and sediment', 'Verify water flow restored'],
    },
    {
      ...
      steps: ['Assess water volume', 'Deploy portable pumps', 'Monitor water level reduction', 'Remove when clear'],
    },
    {
      ...
      department: 'Drainage & Water',
      steps: ['Engineering survey', 'Capacity analysis', 'Submit upgrade proposal', 'Budget planning'],
    },
  ],
  pothole: [
    {
      ...
      steps: ['Clean pothole area', 'Apply tack coat', 'Fill with cold mix', 'Compact and level'],
    },
    {
      ...
      expected_impact: 'immediate',
      department: 'Emergency Services',
      steps: ['Place warning signs', 'Set up cones/barriers', 'Reduce speed limit temporarily', 'Monitor traffic flow'],
    },
    {
      ...
      expected_impact: 'long-term',
      department: 'Roads & Infrastructure',
      steps: ['Surface assessment', 'Plan resurfacing project', 'Schedule road closure', 'Execute resurfacing'],
    },
  ],
  garbage: [
    {
      ...
      steps: ['Dispatch collection truck', 'Remove accumulated waste', 'Clean affected area', 'Verify completion'],
    },
    {
      ...
      department: 'Sanitation',
      steps: ['Assess bin requirements', 'Install new bins', 'Update collection route', 'Inform residents'],
    },
    {
      ...
      expected_impact: 'long-term',
      department: 'Sanitation',
      steps: ['Identify hotspots', 'Deploy patrol units', 'Install surveillance signs', 'Issue penalties if needed'],
    },
  ],
};

const RELATIONSHIP_GRAPHS: Record<string, { nodes: { id: string; label: string }[]; edges: { from: string; to: string; label: string; weight: number }[] }> = {
  flooding: {
    nodes: [
    ...
    edges: [
      ...
      { from: 'garbage', to: 'drainage', label: 'contributes_to', weight: 0.55 },
    ],
  },
  pothole: {
    nodes: [
    ...
    edges: [
      ...
      { from: 'flooding', to: 'water', label: 'causes', weight: 0.70 },
  ...
  garbage: {
    ...
    edges: [
      ...
      { from: 'garbage', to: 'drainage', label: 'contributes_to', weight: 0.55 },
      { from: 'drainage', to: 'flooding', label: 'causes', weight: 0.60 },
    ],
  },
};

function detectProblemFromText(text: string): { type: string; confidence: number } {
...
function extractContext(text: string): Record<string, unknown> {
  ...
  if (/\b(near|around|beside|next to)\b/.test(lower)) context.location_hint = 'relative';
  if (/\b(urgent|emergency|danger|immediate)\b/.test(lower)) context.severity = 'high';
...
export interface AnalysisResult {
  ...
  relationship_graph: { nodes: { id: string; label: string }[]; edges: { from: string; to: string; label: string; weight: number }[] };
  processing_time_ms: number;
}

export async function runAIAnalysis(input: AnalysisInput): Promise<AnalysisResult> {
  ...
  const category = input.categories.find((c) => c.name === problemType);

  const visionConfidence = Math.round((0.75 + pseudoRandom(seed) * 0.22) * 1000) / 1000;
  const textConfidence = textResult.confidence;
  const finalConfidence = Math.round(((visionConfidence * 0.6) + (textConfidence * 0.4)) * 1000) / 1000;

  const authenticityScore = Math.round((0.80 + pseudoRandom(seed + 1) * 0.18) * 1000) / 1000;
  ...
  const recommendations = RECOMMENDATIONS_BY_PROBLEM[problemType] || RECOMMENDATIONS_BY_PROBLEM.flooding;
  const graph = RELATIONSHIP_GRAPHS[problemType] || RELATIONSHIP_GRAPHS.flooding;
  ...
  return {
    ...
    visible_conditions: CONDITIONS_BY_PROBLEM[problemType] || CONDITIONS_BY_PROBLEM.flooding,
...
export function analysisToDbParams(reportId: string, result: AnalysisResult, categoryId: string) {
  return {
    report_id: reportId,
...
export function getDepartmentForProblem(problemType: string, departments: { id: string; name: string }[]): string | null {
